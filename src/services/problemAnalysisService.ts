...
## 三、输出格式（非常重要）
- 整体方针：你的目标不是把所有数字逐条复述，而是筛选出大约5条以上最关键的剪刀差信息，这些信息要能够清晰指向"目前品牌运营中存在的核心问题"，并用简洁、业务化的语言表达出来。
...
**重要**：
- **必须包含四个维度的分析**：环境因素（如集采政策、省份差异、市场环境）、商业推广因素（如渠道覆盖、推广策略、市场投入）、产品因素（如剂量规格、包装大小、产品特性）、资源分配因素（如WD分销率、渠道资源配置）
...
5. **必须包含四个维度的分析**：环境因素（如集采政策、省份差异、市场环境）、商业推广因素（如渠道覆盖、推广策略、市场投入）、产品因素（如剂量规格、包装大小、产品特性）、资源分配因素（如WD分销率、渠道资源配置）
6. **不要按四个因素分类列出**，而是自然地融合在一句连贯的statement中
7. **可以基于domain knowledge补充合理的分析**，但要与查询数据结合
8. **⚠️ 重要：不要在statement中提及金额（如"XX元"、"XX万元"）和份额（如"XX%"、"份额"等）数据**
...
\`\`\`json
{
  "causes": [
    {
      "problem": "${gap.title}",
      "statement": "总结性的分析陈述，必须包含环境因素、商业推广因素、产品因素、资源分配因素四个维度的分析，但不要按四个因素分类列出，而是自然地融合在一句连贯的statement中，必须引用具体数据，但⚠️ 重要：不要在statement中提及金额和份额数据"
    }
  ]
}
\`\`\``
