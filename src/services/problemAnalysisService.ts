...
- 第二步：针对发现的剪刀差，发现其中重复项目，并合并，具体的判断方法
  ...
  - 以立普妥-零售渠道为例，：“零售渠道分子式内份额落后:立普妥相对可定弱势” vs “不同分子式原研品表现分化:立普妥与可定在零售渠道份额剪刀差”，这两条为重复内容，应该合并
...
请以JSON格式输出：
{
  "causes": [
    {
      "problem": "问题描述",
      "statement": "总结性的分析陈述，直接总结归类所有发现，不要按照环境因素、商业推广因素、产品因素、资源分配因素来分类。应该是一句连贯的statement，例如：'医院内，立普妥分子式内份额与可定持平（均为~12%），零售渠道内，立普妥在分子式内份额低于可定（~9%对比~12%）；拆分来看，立普妥主要是10mg中标省份（如安徽、合肥）的份额明显低。同时，发现立普妥10mg中标省份的WD较低（44对比其他省份60），基于WD分销作为零售的重要因素，可能存在进一步提升的空间。'"
    }
  ]
}

**重要**：
- 不要按照环境因素、商业推广因素、产品因素、资源分配因素来分类输出
- 直接总结归类，输出一句连贯的statement
- 必须引用查询结果中的具体数据（如数值、省份名称等）
- 所有分析必须基于查询函数返回的真实数据
...
\`\`\`json
{
  "causes": [
    {
      "problem": "剪刀差标题",
      "statement": "总结性的分析陈述，直接总结归类所有发现，例如：'医院内，立普妥分子式内份额与可定持平（均为~12%），零售渠道内，立普妥在分子式内份额低于可定（~9%对比~12%）；拆分来看，立普妥主要是10mg中标省份（如安徽、合肥）的份额明显低。同时，发现立普妥10mg中标省份的WD较低（44对比其他省份60），基于WD分销作为零售的重要因素，可能存在进一步提升的空间。'"
    }
  ]
}
\`\`\`
...
3. **必须引用具体的数值**，如"立普妥10mg的WD为40，20mg的WD为50"
4. **必须进行多维度交叉分析**，结合不同查询结果
5. **不要按照环境因素、商业推广因素、产品因素、资源分配因素来分类输出**
6. **直接总结归类，输出一句连贯的statement**
...
\`\`\`json
{
  "causes": [
    {
      "problem": "${gap.title}",
      "statement": "总结性的分析陈述，直接总结归类所有发现，必须引用具体数据，例如：'医院内，立普妥分子式内份额与可定持平（均为~12%），零售渠道内，立普妥在分子式内份额低于可定（~9%对比~12%）；拆分来看，立普妥主要是10mg中标省份（如安徽、合肥）的份额明显低。同时，发现立普妥10mg中标省份的WD较低（44对比其他省份60），基于WD分销作为零售的重要因素，可能存在进一步提升的空间。'"
    }
  ]
}
\`\`\`
...
**要求**：
1. 必须基于上述查询结果进行深度分析
2. 必须引用查询结果中的具体数据（如"立普妥10mg的WD为40"）
3. 不能使用占位符文本
...
\`\`\`json
{
  "causes": [
    {
      "problem": "${gap.title}",
      "statement": "总结性的分析陈述，直接总结归类所有发现，必须引用具体数据"
    }
  ]
}
\`\`\``
