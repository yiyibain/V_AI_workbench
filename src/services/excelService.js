 ...
 */
export async function readExcelFile(filePath, filterByValue = true) {
    try {
        ...
        // 创建维度配置
        const dimensionConfigs = dimensionColumns.map((col, idx) => ({
            key: `dimension${idx + 1}`,
            label: col,
        ...
        for (let i = 1; i < jsonData.length; i++) {
            ...
            const pdotIndex = cleanedHeaders.findIndex(h => h?.toLowerCase().trim() === 'pdot');
 ...
 * 推断维度类型
 */
function inferDimensionType(columnName) {
