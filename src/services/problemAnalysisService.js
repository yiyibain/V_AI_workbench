...
// 格式化市场数据为AI可理解的格式
function formatMarketDataForAI(marketData, mekkoData, selectedXAxisKey, selectedYAxisKey, availableDimensions, selectedBrand) {
    ...
    // 提取品牌维度
    const brandDimension = availableDimensions.find(d => d.label.toLowerCase().includes('品牌') || d.label.toLowerCase().includes('brand'));
    // 统计各维度的数据
    const summary = [];
    summary.push(`## 数据概览`);
    ...
    if (brandDimension) {
        ...
        marketData.forEach(point => {
            const brand = point[brandDimension.key] || '';
            if (brand) {
...
function getDimensionKeyByLabel(availableDimensions, labelKeywords) {
...
// 通用模糊匹配函数（用于所有维度的查询）
function fuzzyMatch(dataValue, targetValue) {
...
function fuzzyMatchBrand(dataBrand, targetBrand) {
    return fuzzyMatch(dataBrand, targetBrand);
}
// 执行数据查询函数
// 注意：所有查询都使用模糊匹配（fuzzyMatch），包括品牌、省份、规格、剂量、价格等维度
async function executeDataQuery(functionName, args, _marketData, // 保留参数以保持接口一致性，但实际使用数据库文件
_availableDimensions, // 保留参数以保持接口一致性，但实际使用数据库文件的维度
selectedBrand) {
    try {
        ...
        switch (functionName) {
                ...
                if (dosage === 'all') {
                    ...
                    filteredData.forEach(point => {
                        ...
                        if (brand) {
                            ...
                            if (brandKey && fuzzyMatchBrand(point[brandKey], brand)) {
                            ...
                            const brandKey = getDimensionKeyByLabel(queryDimensions, ['品牌', 'brand']);
                            if (brandKey && fuzzyMatchBrand(point[brandKey], selectedBrand)) {
                                stats.brandTotal += point.value || 0;
                            }
                        }
                    ...
                    if (brand) {
                        const brandKey = getDimensionKeyByLabel(queryDimensions, ['品牌', 'brand']);
                        if (brandKey) {
                            filteredData = filteredData.filter(point => fuzzyMatchBrand(point[brandKey], brand));
                        }
                    }
                    else {
                        const brandKey = getDimensionKeyByLabel(queryDimensions, ['品牌', 'brand']);
                        if (brandKey) {
                            filteredData = filteredData.filter(point => fuzzyMatchBrand(point[brandKey], selectedBrand));
                        }
                    }
            ...
            case 'queryWD': {
                ...
                if (brandKey) {
                    ...
                    filteredData = filteredData.filter(point => {
                        const brandValue = point[brandKey];
                ...
                if (packageSize && packageKey) {
                    filteredData = filteredData.filter(point => fuzzyMatch(String(point[packageKey] || ''), packageSize));
                }
...
// 调用AI进行剪刀差分析
export async function analyzeScissorsGaps(marketData, mekkoData, selectedXAxisKey, selectedYAxisKey, availableDimensions, selectedBrand, maxItems = 5) {
    const formattedData = formatMarketDataForAI(marketData, mekkoData, selectedXAxisKey, selectedYAxisKey, availableDimensions, selectedBrand);
...
// 分析问题原因和策略
export async function analyzeProblemsAndStrategies(scissorsGaps, selectedBrand, marketData, availableDimensions, userFeedback, maxProblems = 10, confirmedProblems) {
    const gapsText = scissorsGaps.map((gap, idx) => `${idx + 1}. ${gap.title}\n   现象：${gap.phenomenon}${gap.possibleReasons ? `\n   可能原因：${gap.possibleReasons}` : ''}`).join('\n\n');
                    ...
                    while (iteration < maxIterations) {
                        ...
                        if (message.tool_calls && message.tool_calls.length > 0) {
                            ...
                            for (const toolCall of message.tool_calls) {
                                ...
                                const queryResult = await executeDataQuery(functionName, functionArgs, marketData, availableDimensions, selectedBrand);
